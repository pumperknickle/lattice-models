        ...
        describe("Transaction") {
            ...
            describe("Every transaction should have valid signatures") {
                it("should reject transactions with invalid signatures") {
                    ...
                    let fakeSigTransaction = TransactionArtifactType.TransactionType(unreservedActions: convertedTransaction!.unreservedActions, accountActions: convertedTransaction!.accountActions, receiptActions: convertedTransaction!.receiptActions, depositActions: convertedTransaction!.depositActions, genesisActions: convertedTransaction!.genesisActions, seedActions: convertedTransaction!.seedActions, peerActions: convertedTransaction!.peerActions, previousHash: convertedTransaction!.previousHash, parentReceipts: convertedTransaction!.parentReceipts, signers: convertedTransaction!.signers, signatures: fakeSignature, fee: convertedTransaction!.fee, parentHomesteadRoot: convertedTransaction!.parentHomesteadRoot, transactionHash: convertedTransaction!.transactionHash, stateDelta: convertedTransaction!.stateDelta, stateData: convertedTransaction!.stateData)
